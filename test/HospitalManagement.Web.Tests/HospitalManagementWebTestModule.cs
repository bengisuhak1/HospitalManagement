...
[DependsOn(
    ...
    typeof(HospitalManagementWebModule),
    typeof(HospitalManagementApplicationTestModule),
    typeof(HospitalManagementEntityFrameworkCoreTestModule)
)]
public class HospitalManagementWebTestModule : AbpModule
{
    public override void PreConfigureServices(ServiceConfigurationContext context)
    {
        context.Services.PreConfigure<IMvcBuilder>(builder =>
        {
            builder.PartManager.ApplicationParts.Add(new CompiledRazorAssemblyPart(typeof(HospitalManagementWebModule).Assembly));
        });
